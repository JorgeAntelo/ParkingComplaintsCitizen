...
var newProjectRoot = angular.newProjectRoot || 'projects';
var appendUrl = '?v=' + buildVersion;
const getNestedObject = (nestedObj, pathArr) => {
    return pathArr.reduce((obj, key) =>
        (obj && obj[key] !== 'undefined') ? obj[key] : undefined, nestedObj);
}
const relativePath = getNestedObject(angular, [newProjectRoot, defaultProject, 'architect', 'build', 'options', 'outputPath']); // to identify relative build path when angular make build
...
fs.readdir(__dirname + buildPath, (err, files) => {
    files.forEach(file => {
        if (file.match(file.match(/^(es2015-polyfills|main|polyfills|runtime|scripts|inline|styles)([a-z0-9.\-]*)(js|css)/g))) { // regex is identified by build files generated
